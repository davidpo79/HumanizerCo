...
**Copy protection (all variants):** On every page, the output panel blocks manual text selection (`user-select: none`), right-click (`contextmenu` blocked), and Ctrl+C (intercepted and rerouted through the Copy button). The Copy button always checks signup status — gate appears if not yet signed up, regardless of whether the run was a sample or the user's own text.
...
**What changes:** The blur gate is removed entirely. The full humanized output is always visible — no truncation, no blur strip. A Copy button appears at the bottom of the output panel once the text finishes typing. When the user clicks Copy, a modal appears asking for their email ("Enter your email to copy — free, no credit card"). The CTA is "Unlock & Copy →".
...
**What changes:** The gate overlay gains two OAuth buttons above the email field: "Continue with Google" and "Continue with Apple." The email form remains as a fallback below an "OR" divider. The blur gate triggers at the same 65% point as the control.
...
Test 3 (Social OAuth) has the most upside on mobile but also the most uncertainty about audience fit, so I'd run it second when we have a cleaner baseline. Test 1 (Zero-Click Aha) is the most experimental — it removes user agency entirely — and I'd run it last, with a short bounce-rate kill switch ready if the first-10-second exit rate spikes.
